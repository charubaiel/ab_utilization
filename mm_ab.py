...
def generate_data(success_rate: float = 0.1,
                  uplift: float = 0.1,
                  N: int = 5000,
                  skew: float = 1.0,
                  beta: float = 100.,
    ...
    alpha_1 = success_rate * (1 + uplift) * beta / (1 - success_rate *
                                                    (1 + uplift))
...
class BaseABMethod:
    ...
    def ci_check(self) -> dict:
        ...
        std_diff = (
            self.control_ctr.var() / self.control_ctr.shape[0] +
            self.threatment_ctr.var() / self.threatment_ctr.shape[0])**(1 / 2)

        left_bound, right_bound = sps.norm(scale=std_diff,
                                           loc=effect).ppf([0.025, 0.975])
        ci_lenght = right_bound - left_bound

        t = effect / std_diff
        pvalue_tt = 2 * (1 - sps.norm.cdf(np.abs(t)))

        pvalue_mw = sps.mannwhitneyu(self.control_ctr,
                                     self.threatment_ctr).pvalue
...
class ABBootstrap(BaseABMethod):
    ...
    def ci_check(self, n_bootstrap=1000):
        bts_shape = np.minimum(self.control_clicks.shape[0],
        ...
        poisson_bootstraps = sps.poisson(1).rvs(
            (n_bootstrap, bts_shape)).astype(np.int64)

        ctr_control = np.matmul(self.control_clicks,
                                poisson_bootstraps.T) / np.matmul(
                                    self.control_views, poisson_bootstraps.T)
        ctr_threatment = np.matmul(
            self.threatment_clicks, poisson_bootstraps.T) / np.matmul(
                self.threatment_views, poisson_bootstraps.T)

        left_bound, right_bound = np.quantile(ctr_threatment - ctr_control,
                                              [0.025, 0.975])
        ci_lenght = right_bound - left_bound
        ...
        pvalue_tt = 2 * np.minimum(bts_diff,
                                   n_bootstrap - bts_diff) / n_bootstrap
...
class ABBucketing(BaseABMethod):
    ...
    def calc_metric(self, bucket_size=100) -> None:
        min_range = np.minimum(self.control_views.shape[0],
                               self.threatment_views.shape[0])
...
class ABBayes(BaseABMethod):
    ...
    def plot_density(self):
        beta_control = sps.beta(self.control_clicks.sum() + 1,
                                self.control_views.sum() + 1)
        beta_threatment = sps.beta(self.threatment_clicks.sum() + 1,
                                   self.threatment_views.sum() + 1)

        beta_df = pd.DataFrame(np.vstack(
            [beta_control.rvs(10000),
             beta_threatment.rvs(10000)]).T,
                               columns=['control', 'threatment'])
        g = sns.jointplot(x=beta_df.control,
                          y=beta_df.threatment,
                          kind='kde',
                          n_levels=15)
        g.ax_joint.plot(
            [beta_df.min().min(), beta_df.max().max()],
            [beta_df.min().min(), beta_df.max().max()])
        return g
...
def generate_tests(control_views: np.ndarray, control_clicks: np.ndarray,
                   threatment_views: np.ndarray,
                   threatment_clicks: np.ndarray):
    result = {}
    result.update({
        'base':
        BaseABMethod(control_views, control_clicks, threatment_views,
                     threatment_clicks).ci_check()
    })
    result.update({
        'reweight':
        ABReweight(control_views, control_clicks, threatment_views,
                   threatment_clicks).ci_check()
    })
    result.update({
        'linearization':
        ABLinearize(control_views, control_clicks, threatment_views,
                    threatment_clicks).ci_check()
    })
    result.update({
        'bucketing':
        ABBucketing(control_views, control_clicks, threatment_views,
                    threatment_clicks).ci_check()
    })
    result.update({
        'bootstrap':
        ABBootstrap(control_views, control_clicks, threatment_views,
                    threatment_clicks).ci_check()
    })
    result.update({
        'bayesian':
        ABBayes(control_views, control_clicks, threatment_views,
                threatment_clicks).ci_check()
    })
